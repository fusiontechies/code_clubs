...
export default function PreviousPage() {
    ...
    return (
        <div className="min-h-screen bg-white-200 text-white dark:bg-gray-900 dark:text-white bg-white text-black">
            <div className="sticky top-0 bg-white dark:bg-gray-800 p-4 shadow-md z-50 text-black dark:text-white">
                ...
                <div className="flex flex-wrap gap-4 justify-center items-center">
                    <div className="flex items-center gap-4">
                        <input
                            ...
                            className="px-4 py-2 rounded bg-white-200 dark:bg-gray-700 text-dark dark:text-white focus:ring-2 focus:ring-blue-500 w-full sm:w-64 dark:bg-gray-700 dark:text-white bg-gray-300 text-black"
                        />
                        <button
                ...
                {isDatePickerVisible && (
                    <div className='text-center mt-4'>
                        <input
                            ...
                            className="px-4 py-2 rounded bg-white-200 dark:bg-gray-700 text-dark dark:text-white focus:ring-2 focus:ring-blue-500 w-full sm:w-48 custom-calendar dark:bg-gray-700 dark:text-white bg-gray-300 text-black"
                        />
                        {/* Show All Button */}
                        <button
                        ...
                        <div className="flex flex-wrap gap-2 justify-center mb-4 mt-4">
                            {categories.map(category => (
                                <label key={category} className={`flex items-center cursor-pointer ${selectedCategories.includes(category) ? 'bg-blue-600 text-white' : 'bg-gray-200 text-dark dark:bg-gray-700'} text-dark dark:text-white rounded-lg p-2`}>
                                    <input
                                        type="checkbox"
                                        checked={selectedCategories.includes(category)}
                                        onChange={() => toggleCategory(category)}
                                        className="mr-2 bg-white-800 dark:bg-gray-800 text-dark dark:text-white"
                                    />
                                    {category}
                                </label>
                            ))}
            ...
            <div className="p-4 grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                {filteredBlogs.length > 0 ? (
                    filteredBlogs.map((blog) => (
                        <div key={blog._id} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-lg border border-gray-700 dark:bg-gray-800 bg-gray-200 text-black dark:text-white">
